...
HdCyclesMesh::_PopulateNormals(HdSceneDelegate* sceneDelegate, const SdfPath& id)
{
    ...
    if (interpolation == HdInterpolationConstant) {
        ...
        if (m_cyclesMesh->get_use_motion_blur() && m_cyclesMesh->get_motion_steps() > 1) {
            _PopulateMotionAttributeVec3f(sceneDelegate, id, HdTokens->normals, HdPrimvarRoleTokens->normal,
                                          ...
                                          ccl::ATTR_STD_MOTION_CORNER_NORMAL, m_cyclesMesh->get_motion_steps());
        }
#endif
        ...
        if (m_cyclesMesh->get_use_motion_blur() && m_cyclesMesh->get_motion_steps() > 1) {
            _PopulateMotionAttributeVec3f(sceneDelegate, id, HdTokens->normals, HdPrimvarRoleTokens->normal,
                                          ...
                                          ccl::ATTR_STD_MOTION_VERTEX_NORMAL, m_cyclesMesh->get_motion_steps());
        }
                   ...
                   TfToken const& reprToken)
{
        ...
        if (instancer) {
            ...
            if (newNumInstances != 0) {
                ...
                for (size_t j = 0; j < newNumInstances; ++j) {
                    ...
                    instanceObj->set_visibility(m_visibilityFlags);
    ...
    if (*dirtyBits & HdChangeTracker::DirtyPrimvar) {
        ...
        for (size_t i = 0; i < m_cyclesInstances.size(); ++i) {
            ...
            instance.set_visibility(m_visibilityFlags);
            instance.set_lightgroup(m_cyclesObject->get_lightgroup());
            instance.set_color(m_cyclesObject->get_color());

            std::string asset_name = instancer_id.GetString() + "/" + std::to_string(i);
            instance.set_asset_name(ccl::ustring(asset_name.c_str()));
        }
            ...
            for (auto& desc : descriptors) {
                if (desc.name == HdTokens->displayColor) {
                    ...
                    for (size_t i = 0; i < m_cyclesInstances.size(); ++i) {
                        ...
                        instance.set_color(ccl::make_float3(value[0], value[1], value[2]));
                    }
                    continue;
                }
            }
