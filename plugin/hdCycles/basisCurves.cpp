...
HdCyclesBasisCurves::_PopulateMotion(HdSceneDelegate* sceneDelegate, const SdfPath& id)
{
    ...
    if (numSamples <= 1) {
        m_cyclesHair->set_use_motion_blur(false);
        m_cyclesHair->set_motion_steps(0);
        return;
    }

    m_cyclesHair->set_use_motion_blur(true);
    m_cyclesHair->set_motion_steps(static_cast<unsigned int>(numSamples + ((numSamples % 2) ? 0 : 1)));
...
HdCyclesBasisCurves::_PopulateGenerated()
{
    ...
    if (m_cyclesMesh) {
        ...
        for (size_t i = 0; i < m_cyclesHair->num_curves(); i++) {
            ccl::float3 co = m_cyclesHair->get_curve_keys()[m_cyclesHair->get_curve(i).first_key];
            ...
            for (const HdPrimvarDescriptor& description : interpolation_description.second) {
                ...
                if (primvar_name == usdCyclesTokens->primvarsCyclesObjectLightgroup) {
                    ...
                    if (value.IsHolding<std::string>()) {
                        ...
                        m_cyclesObject->set_lightgroup(ccl::ustring(lightGroup.c_str()));
                    }
                    continue;
                }
                ...
                if (primvar_name == usdCyclesTokens->primvarsCyclesObjectPass_id) {
                    ...
                    if (value.IsHolding<bool>())
                        m_cyclesObject->set_pass_id(value.Get<bool>());
                    continue;
                }
...
HdCyclesBasisCurves::_CreateCurves(ccl::Scene* a_scene)
{
    ...
    if ((m_cyclesHair->get_curve_keys().size() != num_keys) || (m_cyclesHair->num_curves() != num_curves)) {
