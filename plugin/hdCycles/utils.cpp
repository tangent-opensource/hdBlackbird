                     ...
                     bool use_motion)
{
    ...
    if (object->get_geometry() && object->get_geometry()->get_motion_steps() == sampleCount) {
        ...
        const float motionStepSize = (xf.times.back() - xf.times.front()) / (numMotionSteps - 1);
...
{
    return ccl::make_float4((float)a_vec[0], (float)a_vec[1], (float)a_vec[2], (float)a_vec[3]);
}
...
struct MikkUserData {
    MikkUserData(const char* layer_name, ccl::Mesh* mesh, ccl::float3* tangent, float* tangent_sign)
        : mesh(mesh)
        , corner_normal(NULL)
        , vertex_normal(NULL)
        , texface(NULL)
